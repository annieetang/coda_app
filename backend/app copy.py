    ...
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                ...
                score = converter.parse(file_path_to_save)

                print("after converter parsed")

                # print("after converter parsed")
                # # Save uploaded file to UPLOAD_FOLDER
                # with open(file_path_to_save, 'w') as f:
                #     f.write(score.write(fmt='musicxml'))
...
def get_music21_from_music_matrix_representation(MMR):
    ...
    if MMR.key_signature:
        part.insert(0, MMR.key_signature)
    if MMR.time_signature:
    ...
    for t in range(num_timesteps):
        ...
        # for midi_pitch in range(num_pitches):
        #     if MMR.durations_matrix[midi_pitch, t] > 0:
        #         new_note = note.Note(midi_pitch)
        #         new_note.quarterLength = MMR.durations_matrix[midi_pitch, t] / MMR.quantization
        #         part.insert(t / MMR.quantization, new_note)
        #         # NOTE: removed the natural, but this might be wrong in certain cases...
        #         if new_note.pitch.accidental == pitch.Accidental('natural'):
...
# TODO: probably pass down the time_signature?
class ExerciseScore:
    def __init__(self, music21_score):
        ...
        self.key_signature = self.original_stream.recurse().getElementsByClass(key.KeySignature)
        self.time_signature = self.original_stream.recurse().getElementsByClass(meter.TimeSignature)
    ...
    def _extract_parts(self):
        ...
        """
        self.original_stream.show('text')

        # TODO: if the part doesn't have notes, don't process it
        return [ExercisePart(part) for part in self.original_stream.parts]

class ExercisePart(ExerciseScore):
    def __init__(self, music21_part):
        super().__init__(music21_part)
    ...
    def _extract_lines(self):
        ...
        lines = [line for line in split_voices.parts]
        exerciseLines = []
        for l in lines:
            # if there are actual notes, then add it as an ExerciseLine, if not ignore
            if l.recurse().notesAndRests:
                exerciseLines.append(ExerciseLine(l))
        return exerciseLines
        # TODO: also get rid of parts with no notes in it

# TODO: change this name to "Voice" instead of "Line"?????
class ExerciseLine(ExercisePart):
    def __init__(self, music21_line):
        super().__init__(music21_line)
